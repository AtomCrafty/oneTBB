...
namespace trace {
	constexpr int traceSkip = 2;
	constexpr int traceSize = 3;
	...
	struct Allocation
	{
		...
		size_t size;
		bool aligned;
		PVOID allocTrace[traceSize];
		PVOID freeTrace[traceSize];
	};

	static std::map<uintptr_t, Allocation> activeAllocations{};
	static std::multimap<uintptr_t, Allocation> freedAllocations{};
	static std::mutex lock{};

	static const Allocation* findOverlappingActiveAllocation(uintptr_t address, size_t size) {
		auto it = activeAllocations.lower_bound(address + size);
		if (it != activeAllocations.end() && it != activeAllocations.begin()) {
			--it;

			const auto& alloc = it->second;

			if (alloc.address + alloc.size > address) {
				return &alloc;
			}
		}
	...
	// this is slow because it might need to iterate all freed allocations ever
	static const Allocation* findContainingFreedAllocation(uintptr_t address, size_t size) {
		auto it = freedAllocations.lower_bound(address);
		if (it == freedAllocations.end()) {
			return nullptr;
		}

		while (it != freedAllocations.begin()) {
			--it;

			const auto& alloc = it->second;

			if (alloc.address + alloc.size >= address + size) {
				return &alloc;
			}
		}
	...
	static std::string formatStackTrace(const PVOID* trace) {
		std::string string{};
		char address[32];
		...
		if (string.empty()) {
			string = "<invalid stack trace>";
		}

		return string;
	}

	static void trace_alloc(void* pointer, size_t size, bool aligned)
	{
		if (!enable) return;
		...
		const auto overlapped = findOverlappingActiveAllocation(address, size);
		if (overlapped) {
			PVOID trace[traceSize];
			RtlCaptureStackBackTrace(traceSkip, traceSize, trace, nullptr);

			char msg[1024];
			sprintf_s(msg,
				"New allocation at %016llX overlaps existing allocation at %016llX"
				"\n\nfirst allocated at:\n%s"
				"\n\nnew allocated at:\n%s",
				address, overlapped->address,
				formatStackTrace(overlapped->allocTrace).c_str(),
				formatStackTrace(trace).c_str());
			MessageBoxA(nullptr, msg, "tbbmalloc", MB_OK | MB_ICONERROR);
			return;
		}

		const auto it = activeAllocations.insert_or_assign(address, Allocation{ address, size, aligned, {nullptr}, {nullptr} }).first;
		auto& alloc = it->second;

		RtlCaptureStackBackTrace(traceSkip, traceSize, alloc.allocTrace, nullptr);
	}

	static void trace_free(void* pointer, bool aligned)
	{
		if (!enable) return;
		if (!pointer) return;
		...
		std::lock_guard<std::mutex> _(lock);
		const auto address = reinterpret_cast<uintptr_t>(pointer);
		const auto activeIt = activeAllocations.find(address);

		if (activeIt == activeAllocations.end()) {
			const auto freedIt = freedAllocations.find(address);
			if (freedIt != freedAllocations.end()) {
				const auto& freed = freedIt->second;

				char msg[1024];
				sprintf_s(msg,
					"Double free of address %016llX"
					"\n\nallocated at:\n%s"
					"\n\nfirst freed at:\n%s",
					address,
					formatStackTrace(freed.allocTrace).c_str(),
					formatStackTrace(freed.freeTrace).c_str());
				MessageBoxA(nullptr, msg, "tbbmalloc", MB_OK | MB_ICONERROR);
				return;
			}
			...
			char msg[1024];
			sprintf_s(msg,
				"Tried to free a nonexistent allocation at address %016llX"
				"\n\nstack trace:\n%s",
				address,
				formatStackTrace(trace).c_str());
			MessageBoxA(nullptr, msg, "tbbmalloc", MB_OK | MB_ICONERROR);
		...
		RtlCaptureStackBackTrace(traceSkip, traceSize, active.freeTrace, nullptr);

		freedAllocations.emplace(address, active);
		activeAllocations.erase(activeIt);

		if (active.aligned != aligned)
		{
			char msg[1024];
			sprintf_s(msg,
				"Alignment mismatch for free of address %016llX"
				"\n\nallocated as %s at:\n%s"
				"\n\nfreed as %s at:\n%s",
				address,
				(active.aligned ? "aligned" : "unaligned"), formatStackTrace(active.allocTrace).c_str(),
				(aligned ? "aligned" : "unaligned"), formatStackTrace(active.freeTrace).c_str());
			MessageBoxA(nullptr, msg, "tbbmalloc", MB_OK | MB_ICONERROR);
			return;
		}
	}

	TBBMALLOC_EXPORT extern "C" void trace_touch(void* pointer, size_t size)
	{
		if (!enable) return;
		...
		if (IsBadReadPtr(pointer, size)) {
			char msg[1024];
			sprintf_s(msg,
				"Tried to access invalid memory range (%016llX+%016llX)"
				"\n\nstack trace:\n%p\n%p\n%p",
				address, size,
		...
		const auto active = findOverlappingActiveAllocation(address, size);
		if (!active) {
			const auto freed = findContainingFreedAllocation(address, size);

			if (freed) {
				char msg[1024];
				sprintf_s(msg,
					"Touched range (%016llX+%016llX) within freed memory (%016llX+%016llX)"
					"\n\nallocated at:\n%p\n%p\n%p"
					"\n\nfreed at:\n%p\n%p\n%p"
					"\n\naccessed at:\n%p\n%p\n%p",
					address, size, freed->address, freed->size,
					freed->allocTrace[0], freed->allocTrace[1], freed->allocTrace[2],
					freed->freeTrace[0], freed->freeTrace[1], freed->freeTrace[2],
					trace[0], trace[1], trace[2]);
				MessageBoxA(nullptr, msg, "tbbmalloc", MB_OK | MB_ICONERROR);
				return;
			}

			MessageBoxA(nullptr, "No allocation found", "tbbmalloc", MB_OK | MB_ICONERROR);
			return;
		}

		if (address < active->address || address + size > active->address + active->size) {
			char msg[1024];
			sprintf_s(msg,
				"Touched memory range (%016llX+%016llX) is not fully contained within allocation (%016llX+%016llX)"
				"\n\nallocated at:\n%p\n%p\n%p"
				"\n\naccessed at:\n%p\n%p\n%p",
				address, size, active->address, active->size,
				active->allocTrace[0], active->allocTrace[1], active->allocTrace[2],
				trace[0], trace[1], trace[2]);
			MessageBoxA(nullptr, msg, "tbbmalloc", MB_OK | MB_ICONERROR);
			return;
		}
...
extern "C" void* scalable_malloc(size_t size)
{
	...
	if (!ptr) errno = ENOMEM;
	trace::trace_alloc(ptr, size, false);
	return ptr;
}

extern "C" void scalable_free(void* object)
{
	trace::trace_free(object, false);
	internalFree(object);
}
...
extern "C" void* scalable_realloc(void* ptr, size_t size)
{
	...
	if (!ptr)
	{
		...
		trace::trace_alloc(tmp, size, false);
	}
	else if (!size) {
		trace::trace_free(ptr, false);
		internalFree(ptr);
		return nullptr;
	}
	else
...
extern "C" void* scalable_calloc(size_t nobj, size_t size)
{
	...
	trace::trace_alloc(result, arraySize, false);
...
extern "C" int scalable_posix_memalign(void** memptr, size_t alignment, size_t size)
{
	...
	trace::trace_alloc(result, size, true);
	if (!result)
		return ENOMEM;
	*memptr = result;
	return 0;
...
extern "C" void* scalable_aligned_malloc(size_t size, size_t alignment)
{
	...
	trace::trace_alloc(tmp, size, true);
	if (!tmp) errno = ENOMEM;
	return tmp;
}
